/*
    ...
    @brief plik zawierający implementację klasy Game
*/
...
void Game::spawnStaticPoints() {
  ...
  if ((int)this->static_points_.size() < this->MAX_STATIC_POINTS_)
    this->static_points_.push_back(
        StaticPoints(this->randPointType(), this->players_));
}
...
void Game::updatePlayer() {
  ...
  while (Keyboard::isKeyPressed(
      Keyboard::E)) { // sprawdzamy przyciśnięcie przycisku E na klawiaturze
    shooting = true;
  }
  for (auto &it : this->players_) {
    ...
    if (shooting &&
        it.getMass() >
            20) { // sprawdza czy gracz strzelił i ma masę większą niż 20
      int type = StaticPointsTypes::FOOD;
      ...
      this->static_points_.push_back(StaticPoints(
          type,
          it.getRadius() + it.getPlayerPostion().x +
              1.8 * it.getRadius() * cos(direction),
          it.getRadius() + it.getPlayerPostion().y -
              1.8 * it.getRadius() * sin(direction),
          it.getStartingSpeed(),
          direction)); // tworzy nowy obiekt statyczny wystrzliwany przez gracza
      it.loseMass();
    }
    view_center += it.getPlayerPostion();
    view_center +=
        Vector2f(log(it.getMass()) / log(1.05), log(it.getMass()) / log(1.05));
  }
...
void Game::updateCollisionForObject(T &participant, vector<U> &objects,
  ...
  for (auto &it : objects) // iteracja po wszystkich graczach
  {
    if (it.getShape().getGlobalBounds().intersects(
            participant.getShape()
                .getGlobalBounds())) { // sprawdzenie czy punkty się nie
                                       // przecinają
      switch (it.getType()) {
      ...
      case StaticPointsTypes::SPIKES: // jeśli spike'a porównaj mase
        ...
        break;
      default:
        if (participant.getMass() >
            it.getMass() * 1.1) // jeśli jest to gracz lub bot
        {
...
void Game::updateTable(stringstream &ss, stringstream &ss2,
                       Vector2f &view_center) {
  ...
  for (auto &it : players_) {
    view_center += it.getPlayerPostion();
    view_center += Vector2f(it.getRadius() + 30,
                            it.getRadius() + 30); // obliczanie środka ekranu
  }
...
void Game::updateMaxPoints() {
  /// @brief funkcja po zakończeniu gry zapisuje maksymalną ilość punktów do
  /// pliku
