...
void Player::calculateFreshSpawnedSpeed() {
  /// @brief oblicza aktualną prędkość gracza na podstawie masy
  this->speed_ =
      this->starting_speed_ * 3 -
      100 * (double)(clock() - this->getFreshSpawnedTime()) / CLOCKS_PER_SEC;
}
...
void Player::connect(vector<Player> &players) {
  ...
  if (players.size() > 1) {
    if (((double)(actual_time - players[0].getSplitTime()) / CLOCKS_PER_SEC) >
        20.0) { // sprawdza czy od ostatniego podziału minął określony czas
      int mass_ = 0;
      ...
      Player player = Player(old_players_pos.x / players.size(),
                             old_players_pos.y / players.size(), mass_,
                             starting_speed_); // tworzy gracza po połączeniu
...
void Player::split(RenderWindow &window, vector<Player> &players) {
  ...
  if (flag) {
    ...
    for (auto &it : players) {
      if (players.size() + temp_players.size() <
          8) { // sprawdzenie czy nie przekroczono maksymalnej ilości kul
        if (it.getMass() > 30) { // sprawdzenie, czy można podzielić
          players[0].setSplitTime(clock());
          it.splitMass();
          float radius = it.getRadius();
          pair<float, float> relative_pos = getRelativeMousePositon(
              window, players); // pobiera pozycje myszki
          float direction = atan2(-relative_pos.second,
                                  relative_pos.first); // obliczenie kierunku
          Player player =
              Player(it.getPlayerPostion().x + 2.5 * radius * cos(direction),
                     it.getPlayerPostion().y - 2.5 * radius * sin(direction),
                     it.getMass(),
                     starting_speed_); // tworzy nowego gracza wystrzelonego w
                                       // konkretnym kierunku
          player.setFreshSpawnedTime(clock()); // ustawia czas respawnu
          temp_players.push_back(
              player); // wrzuca na wektor tymczasowych graczy
        }
      }
    }
...
void Player::splitBySpike(vector<Player> &players) {
  ...
  /// @param players wektor graczy na mapie
  int max_parts_number = min(
      3,
      (8 -
       (int)players.size())); // maksymalna ilosc kulek jednego gracza wynosi 8
  if (max_parts_number > 0) {
    this->setMass(this->getMass() / (max_parts_number + 1));
    for (int i = 1; i <= max_parts_number; ++i) {
      players[0].setSplitTime(clock());
      Player player =
          Player(this->getPlayerPostion().x + 2 * i * this->getRadius(),
...
pair<float, float> Player::getRelativeMousePositon(RenderWindow &window,
...
void Player::move(RenderWindow &window, vector<Player> &players) {
  ...
  this->shape_.move(cos(direction) * this->speed_,
                    -sin(direction) * this->speed_);

  if ((double)(clock() - this->getFreshSpawnedTime()) / CLOCKS_PER_SEC < 0.3 &&
      (double)(clock() - this->getFreshSpawnedTime()) / CLOCKS_PER_SEC >
          0) // oddzielna reguła dla nowo powstałych kul
    return;

  for (auto &it : players) // iteracja po każdej kuli gracza
  {
    if (this->getShape().getGlobalBounds().intersects(
            it.getShape().getGlobalBounds())) // sprawdzenie czy kule nie
                                              // nachodzą na siebie
    {
      ...
      it.getShape().setPosition(x1 + cos(direction) * d,
                                y1 - sin(direction) * d); // omijanie kuli
    }
  }
}
...
void Player::checkMapCollision() {
  ...
  if (this->shape_.getGlobalBounds().left +
          this->shape_.getGlobalBounds().width >=
      4800) {
    this->shape_.setPosition(4800 - this->shape_.getGlobalBounds().width,
                             this->shape_.getGlobalBounds().top);
  }
  if (this->shape_.getGlobalBounds().top <= -2700) {
    this->shape_.setPosition(this->shape_.getGlobalBounds().left, -2670);
  }
  if (this->shape_.getGlobalBounds().top +
          this->shape_.getGlobalBounds().height >=
      2700) {
    this->shape_.setPosition(this->shape_.getGlobalBounds().left,
                             2700 - this->shape_.getGlobalBounds().height);
  }
}
