...
Button::Button(Vector2f size = Vector2f(125, 60), Color color = Color::Cyan,
               Vector2f pos = Vector2f(560, 500)) {
  /// @brief konstruktor klasy button tworzący obiekt o podanych na wejściu
  /// danych
...
bool Button::isButtonPressed(RenderWindow *window) {
  ...
  IntRect rect(this->button_.getPosition().x, this->button_.getPosition().y,
               this->button_.getGlobalBounds().width,
               this->button_.getGlobalBounds().height);
