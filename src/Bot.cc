...
bool Bot::searchNearby(const vector<T> &participants, int range) {
  ...
  for (auto &it : participants) // iteracja po wszystkich obiektach
  {
    ...
    if (diff_x < range &&
        diff_y < range) // sprawdzenie czy obiekt jest w zasięgu
    {
      ...
      if (this_mass >= 1.2 * it_mass) // porównanie mas
      {
        if (diff_x + diff_y < closest) {
          direction = this->getDirection(
              it_x, it_y); // ustalenie kierunku do najbliższego
...
void Bot::moveBot(const vector<Bot> &bots, const vector<Player> &players,
                  const vector<StaticPoints> &static_points) {
