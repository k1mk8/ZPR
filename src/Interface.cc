...
void Interface::initText() {
  ...
  setNewTextParams(this->standard_text_, Color::Black, 25, Vector2f(280, 100),
                   "\t\t\t\t\t\tWitamy w grze AGARIO!\n Prosze wcisnac "
                   "przycisk graj, aby rozpoczac rozgrywke!");

  // tekst na przycisku
  setNewTextParams(this->button_text_, Color::Black, 25, Vector2f(590, 565),
                   "Graj!");

  // tekst na przycisku prędkości
  setNewTextParams(this->button_text1_, Color::Black, 35, Vector2f(420, 475),
                   "5x");

  // tekst na przycisku prędkości
  setNewTextParams(this->button_text2_, Color::Black, 35, Vector2f(600, 475),
                   "10x");

  // tekst na przycisku prędkości
  setNewTextParams(this->button_text3_, Color::Black, 35, Vector2f(780, 475),
                   "20x");

  // maksymalna ilość punktów w historii
  int all_max_points = 0;
  ifstream Plik("./maxPoints.txt"); // pobieranie danych z pliku o maksymalnej
                                    // ilości punktów
  Plik >> all_max_points;
  Plik.close();
  auto s = to_string(all_max_points);
  setNewTextParams(this->max_points_text_, Color::Black, 25, Vector2f(380, 425),
                   "Twoj najlepszy dotychczasowy wynik to: " + s);
}
