...
public:
  Player(float x = 0, float y = 0, const int mass = 10,
         const int speed = 10);                    // konstruktor graczy
  ...
  void calculateFreshSpawnedSpeed(); // obliczanie prędkości po utworzeniu
                                     // nowego bytu
  void setMass(const int weight);    // setter masy
  void grow(const int food);         // jedzenie masy
  void split(sf::RenderWindow &window,
             std::vector<Player> &players);        // dzielenie gracza
  void splitBySpike(std::vector<Player> &players); // dzielenie przez spike'a
  std::pair<float, float> getRelativeMousePositon(sf::RenderWindow &window,
                                                  std::vector<Player> &players);
  void move(sf::RenderWindow &window,
            std::vector<Player> &players); // ruch gracza
  ...
  void setPosition(std::vector<Player> &players,
                   sf::RenderWindow &window); // ustawianie pozycji
