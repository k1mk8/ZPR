...
public:
  ...
  void
  updateCollisionForObject(T &participant, std::vector<U> &objects,
                           std::vector<T> &participants); // sprawdzanie kolizji
  void
  updateTable(std::stringstream &ss, std::stringstream &ss2,
              sf::Vector2f &view_center); // aktualizowanie tablicy z wynikami
